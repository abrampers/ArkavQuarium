...
void Game::initState() {
  ...
  aquarium =
      new Aquarium(aquariumXStart, aquariumYStart, aquariumXEnd, aquariumYEnd);
  game_start_time = graphics.timeSinceStart();
}

/* Load game state from an external file */
void Game::loadState(string filename) { /* TODO */ }

/* Save game state to an external file */
void Game::saveState(string filename) { /* TODO */ }
...
GameState Game::startGame() {
  ...
  while (running) {
    ...
    int aquarium_click_target = graphics.addClickTarget(
        aquariumXStart, aquariumXEnd, aquariumYStart, aquariumYEnd);

    /* Register UI click targets */
    int buy_guppy_click_target =
        graphics.addClickTarget(buyGuppyButtonXStart, buyGuppyButtonXEnd,
                                buyGuppyButtonYStart, buyGuppyButtonYEnd);

    int buy_piranha_click_target =
        graphics.addClickTarget(buyPiranhaButtonXStart, buyPiranhaButtonXEnd,
                                buyPiranhaButtonYStart, buyPiranhaButtonYEnd);

    int buy_snail_click_target =
        graphics.addClickTarget(buySnailButtonXStart, buySnailButtonXEnd,
                                buySnailButtonYStart, buySnailButtonYEnd);

    int buy_egg_click_target =
        graphics.addClickTarget(buyEggButtonXStart, buyEggButtonXEnd,
                                buyEggButtonYStart, buyEggButtonYEnd);
    ...
    for (int i = 0; i < guppy_list.getLength(); i++) {
      ...
      graphics.drawGuppy(curr_guppy_x, curr_guppy_y, curr_guppy_level,
                         curr_guppy_state, curr_guppy_state_progress,
                         curr_guppy_hungry);
    }
    ...
    for (int i = 0; i < piranha_list.getLength(); i++) {
      ...
      graphics.drawPiranha(curr_piranha_x, curr_piranha_y, curr_piranha_state,
                           curr_piranha_state_progress, curr_piranha_hungry);
    }
    ...
    for (int i = 0; i < snail_list.getLength(); i++) {
      ...
      graphics.drawSnail(curr_snail_x, curr_snail_y, curr_snail_state,
                         curr_snail_state_progress);
    ...
    for (int i = 0; i < coin_list.getLength(); i++) {
      ...
      graphics.drawCoin(curr_coin_x, curr_coin_y, curr_coin_gold,
                        curr_coin_state_progress);
      int click_target = graphics.addClickTarget(
          curr_coin_x - coinClickRadius, curr_coin_x + coinClickRadius,
          curr_coin_y - coinClickRadius, curr_coin_y + coinClickRadius);
    ...
    } else if (coin < guppyPrice && guppy_list.getLength() == 0 &&
               piranha_list.getLength() == 0 && coin_list.getLength() == 0) {
      running = false;
    ...
    /* DEBUG */
    // cout << "FPS: " << 1.0 / (graphics.timeSinceStart() - game_current_time)
    // << endl;
  }
...
GameState Game::showMainMenu() {
  ...
  graphics.resetClickTargets();
  int start_click_target =
      graphics.addClickTarget(mainStartButtonXStart, mainStartButtonXEnd,
                              mainStartButtonYStart, mainStartButtonYEnd);
...
GameState Game::showWinMenu() {
  ...
  graphics.resetClickTargets();
  int start_click_target =
      graphics.addClickTarget(winStartButtonXStart, winStartButtonXEnd,
                              winStartButtonYStart, winStartButtonYEnd);
...
GameState Game::showloseMenu() {
  ...
  graphics.resetClickTargets();
  int start_click_target =
      graphics.addClickTarget(loseStartButtonXStart, loseStartButtonXEnd,
                              loseStartButtonYStart, loseStartButtonYEnd);
