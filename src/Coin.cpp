...
Coin::Coin(double x, double y, int value, Aquarium *aquarium)
    : Aquatic(x, y, coinMoveSpeed, aquarium), value(value) {
  last_bottom_time = 0;
...
void Coin::move() {
  ...
  double dy =
      this->getMoveSpeed() *
      ((current_time - this->getLastCurrTime()));  // Asumsi waktu dalam ms
  if (this->isInside()) {
...
void Coin::dead() { this->getAquarium()->deleteCoin(this); }