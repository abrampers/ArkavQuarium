...
Guppy::Guppy(Aquarium* aquarium)
    : Aquatic(floor(fRand(aquarium->getXMin(), aquarium->getXMax())),
              ...
              guppyMoveSpeed, aquarium),
      Fish(guppyFoodThres, guppyEatRadius, guppyFullInterval,
           guppyHungerInterval, aquarium->getCurrTime()) {
  this->nearest_pellet = nullptr;
...
double Guppy::distanceToPellet(Pellet* p) {
  ...
  return sqrt((guppy_x_position - pellet_x_position) *
                  (guppy_x_position - pellet_x_position) +
              (guppy_y_position - pellet_y_position) *
                  (guppy_y_position - pellet_y_position));
}

void Guppy::findNearestPellet() {
  LinkedList<Pellet*>& ll = this->getAquarium()->getPelletList();
  Pellet* current_nearest_pellet = nullptr;
  Node<Pellet*>* curr_node = ll.getHead();
  while (curr_node != nullptr) {
    Pellet* current_pellet = curr_node->getValue();
    if (current_nearest_pellet == nullptr) {
      current_nearest_pellet = current_pellet;
    } else if (distanceToPellet(current_pellet) <
               distanceToPellet(current_nearest_pellet)) {
      current_nearest_pellet = current_pellet;
    }
...
void Guppy::updateState() {
  ...
  if (this->getState() == State::deadLeft ||
      ...
      (this->getHungry() &&
       current_time - this->getLastHungerTime() > this->hungerTimeout)) {
    /* Dead guppy */
    this->dead();
  } else {
    this->updateProgress();
...
void Guppy::move() {
  ...
  if (this->getState() != turningRight && this->getState() != turningLeft) {
    if (nearest_pellet != nullptr && this->getHungry()) {
      ...
      double dx = (x_direction / distance) * this->getMoveSpeed() *
                  ((current_time - this->getLastCurrTime()));
      double dy = (y_direction / distance) * this->getMoveSpeed() *
                  ((current_time - this->getLastCurrTime()));
      ...
      /* Continue movement */
      double dx = this->x_dir * this->getMoveSpeed() *
                  (current_time - this->getLastCurrTime());
      double dy = this->y_dir * this->getMoveSpeed() *
                  (current_time - this->getLastCurrTime());
...
void Guppy::eat() {
  double current_time = this->getAquarium()->getCurrTime();
  if (!this->getHungry() &&
      (current_time - this->getLastEatTime() > this->fullInterval)) {
...
void Guppy::dropCoin() {
  ...
  if (current_time - this->last_drop_coin > guppyCoinInterval) {
    this->getAquarium()->createCoin(this->getX(), this->getY(),
                                    this->getLevel() * guppyCoinMultiplier);
...
void Guppy::updateProgress() {
  ...
  if (this->getHungry() &&
      ...
      (this->nearest_pellet != nullptr) &&
      distanceToPellet(this->nearest_pellet) < (2 * guppyEatRadius)) {
...
void Guppy::dead() {
  if (this->getState() == movingRight ||
      (this->getState() == turningRight && this->getProgress() >= 5) ||
      (this->getState() == turningLeft && this->getProgress() < 5)) {
    this->setState(State::deadRight);
  } else if (this->getState() == movingLeft ||
             (this->getState() == turningLeft && this->getProgress() >= 5) ||
             (this->getState() == turningRight && this->getProgress() < 5)) {
    this->setState(State::deadLeft);
  }
  double current_time = this->getAquarium()->getCurrTime();
  if (current_time - this->getLastProgressTime() >
      guppyDeadProgressIncrementTime) {
    this->setProgress(this->getProgress() + 1);
    this->setLastProgressTime(current_time);
