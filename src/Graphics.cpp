...
bool Graphics::init() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    sdl_window = SDL_CreateWindow("ArkavQuarium", SDL_WINDOWPOS_UNDEFINED,
...
void Graphics::drawTopBar(int coin_count, int egg_count) {
  ...
  /* Draw texts */
  drawText(to_string(coin_count), coinTextSize, coinTextX, coinTextY,
           coinTextColorR, coinTextColorG, coinTextColorB);
  ...
  drawText(to_string(piranhaPrice), priceTextSize, piranhaPriceTextX,
           piranhaPriceTextY, priceTextColorR, priceTextColorG,
           priceTextColorB);
  ...
  drawText(to_string(eggPrice), priceTextSize, eggPriceTextX, eggPriceTextY,
           priceTextColorR, priceTextColorG, priceTextColorB);

  drawText(to_string(egg_count), eggCountTextSize, eggCountTextX, eggCountTextY,
           eggCountTextColorR, eggCountTextColorG, eggCountTextColorB);
}
...
void Graphics::drawGuppy(int x, int y, int level, State state,
                         int state_progress, bool hungry) {
...
void Graphics::clearScreen() {
  SDL_FillRect(g_screen_surface, nullptr,
               SDL_MapRGB(g_screen_surface->format, 255, 255, 255));
}

void Graphics::updateScreen() { SDL_UpdateWindowSurface(sdl_window); }
    ...
    if (event.type == SDL_QUIT) {
      ...
      if (event.button.button == SDL_BUTTON_LEFT) {
        for (int i = 0; i < click_targets.size(); i++) {
          ...
          if (mouse_x >= get<0>(target) && mouse_x <= get<1>(target) &&
              mouse_y >= get<2>(target) && mouse_y <= get<3>(target)) {
            clicked_target = i;
          }
        }
      }
...
bool Graphics::quitPressed() { return quit; }
...
const set<SDL_Keycode>& Graphics::getPressedKeys() { return pressed_keys; }

// Mengembalikan himpunan kode tombol yang baru mulai ditekan pada saat
// handle_input() terakhir dipanggil.
const set<SDL_Keycode>& Graphics::getTappedKeys() { return tapped_keys; }
...
int Graphics::getClickedTarget() { return clicked_target; }

int Graphics::getMouseX() { return mouse_x; }

int Graphics::getMouseY() { return mouse_y; }
