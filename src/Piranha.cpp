...
Piranha::Piranha(Aquarium* aquarium)
    : Aquatic(floor(fRand(aquarium->getXMin(), aquarium->getXMax())),
              ...
              piranhaMoveSpeed, aquarium),
      Fish(piranhaFoodThres, piranhaEatRadius, piranhaFullInterval,
           piranhaHungerInterval, aquarium->getCurrTime()) {
...
double Piranha::distanceToGuppy(Guppy* g) {
  ...
  return sqrt((piranha_x_position - guppy_x_position) *
                  (piranha_x_position - guppy_x_position) +
              (piranha_y_position - guppy_y_position) *
                  (piranha_y_position - guppy_y_position));
}
...
void Piranha::findNearestGuppy() {
  ...
  while (curr_node != nullptr) {
    ...
    } else if (distanceToGuppy(current_guppy) <
               distanceToGuppy(current_nearest_guppy)) {
      current_nearest_guppy = current_guppy;
    }
...
void Piranha::updateProgress() {
  ...
  if (this->getHungry() &&
      ...
      (this->nearest_guppy != nullptr) &&
      distanceToGuppy(this->nearest_guppy) < (2 * piranhaEatRadius)) {
...
void Piranha::dead() {
  ...
  if (current_time - this->getLastProgressTime() >
      piranhaDeadProgressIncrementTime) {
    this->setProgress(this->getProgress() + 1);
    this->setLastProgressTime(current_time);
