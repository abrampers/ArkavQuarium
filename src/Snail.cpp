...
Snail::Snail(Aquarium* a)
    : Aquatic(fRand(a->getXMin(), a->getXMax()), a->getYMax(), snailSpeed, a),
      snailRadius(snailEatRadius) {
...
int Snail::getCoin() { return hold_coin_value; }

void Snail::resetCoin() { this->hold_coin_value = 0; }
...
void Snail::findNearestCoin() {
  ...
  for (int i = 0; i < coin_list.getLength(); i++) {
    ...
    } else if ((getDistance(current_coin, this)) <
               getDistance(current_nearest_coin, this)) {
      current_nearest_coin = current_coin;
    }
  }
...
void Snail::move() {
  ...
  if (this->getState() != turningRight && this->getState() != turningLeft) {
    if (nearest_coin != nullptr) {
      if (!isCoinOnTop()) {
        double dx =
            this->getMoveSpeed() * ((current_time - this->getLastCurrTime()));
...
void Snail::updateProgress() {
  if (this->getState() == State::stillRight ||
      this->getState() == State::stillLeft) {
    this->setProgress(0);
  } else {
    double current_time = this->getAquarium()->getCurrTime();
    double progress_increment_time = (this->getState() == State::movingRight ||
                                      this->getState() == State::movingLeft)
